...
async function search() {
    console.log('search');
    let workTypeEmp = $('#workTypeEmp').val();
    ...
    if (workTypeEmp == 'Special') {
        $('#table_special').show();
        $('#table_normal').hide();
        url = '../model/getAssignedSpacial.php';
        bodyScrap = $('#bodySpecial');
    }
    bodyScrap.empty();
    ...
    try {
        ...
        await setShift();
        // ตรวจสอบว่าข้อมูลมาเป็น array
        if (status == 200) {
            if (Array.isArray(data)) {
                if (workTypeEmp == 'Special') {
                    for (const item of data) {
                        // สร้างแถว HTML สำหรับแสดงข้อมูล
                        let checkin = item.timeIn ? item.timeIn.date.substring(0, 16) : 'ไม่พบข้อมูล';
                        let checkout = item.timeOut ? item.timeOut.date.substring(0, 16) : 'ไม่พบข้อมูล';
                        let jobassign = item.jobassign ? item.jobassign : 'ไม่พบข้อมูล';
                        let job_transfer = item.job_transfer ? item.job_transfer : 'ไม่พบข้อมูล';
                        let classJobtf = job_transfer == 'ไม่พบข้อมูล' ? 'text-danger' : 'text-success';
                        ...
                        if (item.cardcode.length < 5) {
                            alert("มีข้อมูลรหัสบัตรผิดพลาด กรุณาตรวจสอบข้อมูล");
                            continue;
                        }
                        text += `
                    <tr class="text-center">
                    <td>${item.cardcode.slice(-5)}</td>
                        <td>${item.th_name}</td>
                        <td>${item.department}</td>
                        <td class="${classJobassign}">${jobassign}</td>
                        <td class="${classJobtf}">${job_transfer}</td>
                        <td class="${classcheckIn}">${checkin}</td>
                        <td class="${classcheckOut}">${checkout}</td>`;
                        if (jobassign == 'special' && job_transfer == 'ไม่พบข้อมูล') {
                            text += `
                            <td>
                                <input type="checkbox" class="job-checkbox" data-checkOut="${checkout}" data-qrcode="${item.cardcode}" data-th_name="${item.th_name}" data-checkin="${checkin}">
                            </td>`;

                        } else if (item.confirm_job_transfer_status != 'confirm') {
                            text += `<td> <div class='btn btn-warning' onclick="cancelAssign('${item.cardcode}', '${item.th_name}', '${checkin}', '${jobassign}', true)">Cancel</div> </> `;
                        } else {
                            text += `<td> <span class='btn btn-success'> Confirmed </span></td> `;
                        }
                    }
                } else {
                    for (const item of data) {
                    ...
                    <td>${item.cardcode.slice(-5)}</td>
                        ...
                        if (jobassign == 'ไม่พบข้อมูล') {
                            ...
                            text += `<td> <div class='btn btn-warning' onclick="cancelAssign('${item.cardcode}', '${item.th_name}', '${checkin}', '${jobassign}')">Cancel</div> </> `;
                        }
                    }
                }

            } else {
                text = '<tr><td colspan="6">ไม่พบข้อมูล</td></tr>';
            }
        } else {
            text = '<tr><td colspan="6">ไม่พบข้อมูล</td></tr>';
        }
        ...
        $('.job-checkbox').on('change', function () {
            ...
            if ($(this).is(':checked')) {
                checkedIds.push({ th_name, checkIn, checkOut, cardcode });
            } else {
                checkedIds = checkedIds.filter(item => item.th_name !== th_name || item.checkIn !== checkIn);
            }
...
$('#btnSubmit').on('click', async function () {
    $('.overlay').show();
    $('#btnSubmit').hide();
    let jobId = $('#workGroup').val();
    let workTypeEmp = $('#workTypeEmp').val();
    try {
        if (workTypeEmp == 'Special') {
            const url = '../model/assignJobtoEmpSpecial.php';
            const headers = new Headers();
            headers.append('Content-Type', 'application/json');
            const requestData = {
                checkedIds, jobId
            ...
            const url = '../model/assignJobtoEmp.php';
            const headers = new Headers();
            headers.append('Content-Type', 'application/json');
            const requestData = {
                checkedIds, jobId
...
$('#btnCancel').on('click', async function () {
    ...
    $('.overlay').show();
    const url = '../model/cancelAssignJob.php';
    const headers = new Headers();
    headers.append('Content-Type', 'application/json');
    const requestOptions = {
        method: 'POST',
    ...
    console.log(JSON.stringify(cancelObj));
    try {
        const response = await fetch(url, requestOptions);
        const status = response.status;
        const data = await response.json();
...
async function cancelAssign(cardcode, th_name, checkin, jobassign, isSpecial = false) {
    if (isSpecial) {
        cancelObj = {
        ...
        cancelObj = {
            cardcode, th_name, checkin, jobassign, isSpecial: false
        }
        $('#cancelModal').modal('show');
    }
}
